...
class TestMeetingAPI:
    ...
    def test_create_meeting_success(self, client, mock_db):
        ...
        with patch("server.meetings_collection", mock_db["meetings"]), patch(
            "server.participants_collection", mock_db["participants"]
        ):
    ...
    def test_create_meeting_without_name(self, client, mock_db):
        ...
        with patch("server.meetings_collection", mock_db["meetings"]), patch(
            ...
            response = client.post(
                "/api/meetings", json={"hostId": "host123"}, content_type="application/json"
            )
    ...
    def test_create_meeting_missing_host_id(self, client):
        ...
        response = client.post(
            "/api/meetings", json={"name": "Test Meeting"}, content_type="application/json"
        )
    ...
    def test_join_meeting_success(self, client, mock_db):
        ...
        with patch("server.meetings_collection", mock_db["meetings"]), patch(
            ...
            participant = mock_db["participants"].find_one(
                {"meetingId": str(meeting_id), "userId": "user456"}
            )
            assert participant is not None
    ...
    def test_leave_meeting_success(self, client, mock_db):
        ...
        with patch("server.participants_collection", mock_db["participants"]):
            ...
            participant = mock_db["participants"].find_one(
                {"meetingId": meeting_id, "userId": "user456"}
            )
            assert participant is None

    def test_get_participants_success(self, client, mock_db):
        """Test getting meeting participants"""
        with patch("server.participants_collection", mock_db["participants"]), patch(
            "server.users_collection", mock_db["users"]
        ):
            meeting_id = str(ObjectId())
            ...
            user1_id = (
                mock_db["users"]
                .insert_one(
                    {"username": "host", "displayName": "Host User", "createdAt": datetime.now()}
                )
                .inserted_id
            )
