...
class TestUserMeetingIntegration:
    ...
    def test_complete_meeting_flow(self, client, mock_db):
        ...
        with patch('server.users_collection', mock_db['users']), \
            ...
            participant_response = client.post('/api/users', 
            ...
            meeting_response = client.post('/api/meetings', 
            ...
            join_response = client.post(f'/api/meetings/{meeting_id}/join',
            ...
            leave_response = client.post(f'/api/meetings/{meeting_id}/leave',
                                       json={'userId': participant_id},
                                       content_type='application/json')
    ...
    def test_multiple_meetings_same_host(self, client, mock_db):
        ...
        with patch('server.users_collection', mock_db['users']), \
            ...
            # Create host
            host_response = client.post('/api/users', 
                                      json={'username': 'multihost'},
                                      content_type='application/json')
            host_id = host_response.get_json()['userId']
            
            # Create multiple meetings
            meeting1_response = client.post('/api/meetings', 
                                          json={'hostId': host_id, 'name': 'Meeting 1'},
                                          content_type='application/json')
            meeting2_response = client.post('/api/meetings', 
    ...
    def test_user_joins_multiple_meetings(self, client, mock_db):
        ...
        with patch('server.users_collection', mock_db['users']), \
            ...
            # Create users
            host1_response = client.post('/api/users', json={'username': 'host1'}, content_type='application/json')
            host2_response = client.post('/api/users', json={'username': 'host2'}, content_type='application/json')
            user_response = client.post('/api/users', json={'username': 'multiuser'}, content_type='application/json')
            ...
            # Create meetings
            meeting1_response = client.post('/api/meetings', json={'hostId': host1_id}, content_type='application/json')
            meeting2_response = client.post('/api/meetings', json={'hostId': host2_id}, content_type='application/json')
            
            meeting1_id = meeting1_response.get_json()['meetingId']
            meeting2_id = meeting2_response.get_json()['meetingId']
            
            # User joins both meetings
            join1_response = client.post(f'/api/meetings/{meeting1_id}/join', json={'userId': user_id}, content_type='application/json')
            join2_response = client.post(f'/api/meetings/{meeting2_id}/join', json={'userId': user_id}, content_type='application/json')
            
            assert join1_response.status_code == 200
            assert join2_response.status_code == 200
            
            # Verify user is in both meetings
            participants1 = mock_db['participants'].find({'meetingId': meeting1_id, 'userId': user_id})
            participants2 = mock_db['participants'].find({'meetingId': meeting2_id, 'userId': user_id})
...
class TestErrorScenarios:
    ...
    def test_join_meeting_after_ended(self, client, mock_db):
        ...
        with patch('server.users_collection', mock_db['users']), \
            ...
            host_response = client.post('/api/users', json={'username': 'host'}, content_type='application/json')
            user_response = client.post('/api/users', json={'username': 'user'}, content_type='application/json')
            
            host_id = host_response.get_json()['userId']
            user_id = user_response.get_json()['userId']
            
            meeting_response = client.post('/api/meetings', json={'hostId': host_id}, content_type='application/json')
            meeting_id = meeting_response.get_json()['meetingId']
            
            # End meeting
            end_response = client.post(f'/api/meetings/{meeting_id}/end', json={'userId': host_id}, content_type='application/json')
            assert end_response.status_code == 200
            
            # Try to join ended meeting
            join_response = client.post(f'/api/meetings/{meeting_id}/join', json={'userId': user_id}, content_type='application/json')
...
class TestSocketIntegration:
    ...
    def test_socket_join_with_valid_meeting(self, socket_client, client, mock_db):
        ...
        with patch('server.users_collection', mock_db['users']), \
            ...
            host_response = client.post('/api/users', json={'username': 'sockethost'}, content_type='application/json')
            host_id = host_response.get_json()['userId']
    ...
    def test_end_meeting_socket_vs_rest(self, socket_client, client, mock_db):
        ...
        with patch('server.users_collection', mock_db['users']), \
            ...
            # Create meeting
            host_response = client.post('/api/users', json={'username': 'endhost'}, content_type='application/json')
            host_id = host_response.get_json()['userId']
    ...
    def test_chat_message_flow(self, socket_client, client, mock_db):
        ...
        with patch('server.users_collection', mock_db['users']), \
            ...
            # Setup meeting
            host_response = client.post('/api/users', json={'username': 'chathost'}, content_type='application/json')
            user_response = client.post('/api/users', json={'username': 'chatuser'}, content_type='application/json')
...
class TestPerformanceScenarios:
    ...
    def test_large_meeting_participants(self, client, mock_db):
        ...
        with patch('server.users_collection', mock_db['users']), \
            ...
            # Create host
            host_response = client.post('/api/users', json={'username': 'perfhost'}, content_type='application/json')
            host_id = host_response.get_json()['userId']
    ...
    def test_rapid_join_leave_sequence(self, client, mock_db):
        ...
        with patch('server.users_collection', mock_db['users']), \
            ...
            # Setup
            host_response = client.post('/api/users', json={'username': 'rapidhost'}, content_type='application/json')
            user_response = client.post('/api/users', json={'username': 'rapiduser'}, content_type='application/json')
