...
class TestUserAPI:
    ...
    def test_create_user_without_display_name(self, client, mock_db):
        ...
        with patch("server.users_collection", mock_db["users"]):
            response = client.post(
                "/api/users", json={"username": "testuser2"}, content_type="application/json"
            )
    ...
    def test_create_user_duplicate_username(self, client, mock_db):
        ...
        with patch("server.users_collection", mock_db["users"]):
            ...
            response = client.post(
                "/api/users", json={"username": "existinguser"}, content_type="application/json"
            )
    ...
    def test_create_user_missing_username(self, client):
        ...
        response = client.post(
            "/api/users", json={"displayName": "Test User"}, content_type="application/json"
        )
    ...
    def test_get_user_success(self, client, mock_db):
        ...
        with patch("server.users_collection", mock_db["users"]):
            ...
            _user_id = (
                mock_db["users"]
                .insert_one(
                    {"username": "getuser", "displayName": "Get User", "createdAt": datetime.now()}
                )
                .inserted_id
            )
...
class TestUserDataValidation:
    ...
    def test_user_creation_long_username(self, client, mock_db):
        ...
        with patch("server.users_collection", mock_db["users"]):
            long_username = "a" * 100
            response = client.post(
                "/api/users", json={"username": long_username}, content_type="application/json"
            )
